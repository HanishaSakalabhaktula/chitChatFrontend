...
const Edit = () => {
    ...
    const [EditUser, {isLoading, err }] = useEditUserMutation();
    ...
    const handleSubmit = async (e) => {
        ...
        if(!name || !email){
            ...
            //sign up the user
            const id = user._id;
            EditUser({id, name, email, picture: url})
                .then((data) => {
                    if(data){
                        // console.log(data);
    ...
    <div className='w-full h-full flex justify-center items-center bg-[#FFF4E0]'>
      <div className="w-3/4 md:w-1/2 h-3/4">
        ...
        <label className="block">
            ...
            <input type="text" placeholder={user.name} className="mt-1 block w-full px-3 py-2 bg-none border border-[#FFBF9B] rounded-md text-sm shadow-sm placeholder-[#4D4D4D]
            focus:outline-none focus:border-[#FFBF9B] focus:border-[#B46060]
            " onChange={(e) => setName(e.target.value)}/>
        </label>
        <label className="block">
            <span className="block my-2 font-medium text-[#4D4D4D]">Email: </span>
            <input type="email" placeholder={user.email} className="mt-1 block w-full px-3 py-2 bg-none border border-[#FFBF9B] rounded-md text-sm shadow-sm placeholder-[#4D4D4D]
            focus:outline-none focus:border-[#FFBF9B] focus:border-[#B46060]
            " onChange={(e) => setEmail(e.target.value)}/>
        </label>

        <button className='mt-4 py-1 px-3 rounded-lg text-[#FFF4E0] font-bold bg-[#B46060]'>{uploadingfile ? "Updating...": "Update"}</button>
        {
            error && <div>{error}</div>
        }
        </form>
